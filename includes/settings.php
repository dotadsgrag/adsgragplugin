...
?>
	...
	<?php }
	$getexistadsIds = $wpdb->get_results("SELECT `aduid` FROM `wp_xyz_adsg_adunit_settings` "); 
    $getexitval = array();
    foreach($getexistadsIds as $sdf)
    {
		
	$getexitval[] = $sdf->aduid;
	}
	
	$adunits1 = get_adunits($xyz_username,$xyz_key);
	$adu_arr1 = json_decode($adunits1);
	
	foreach($getexitval as $val)
    {
        if(!in_array($val, $new_ads_id))
        {
			$wpdb->query('DELETE  FROM '.$wpdb->prefix.'xyz_adsg_adunit_settings WHERE aduid = "'.$val.'"');
		}
         
     } 
	
	foreach($new_ads_id as $val)
    {
        if(!in_array($val, $getexitval))
        {
			$name =  $adu_arr1[1]->{$val}[0];
			$audit =  $val;
			$site_id =  $adu_arr1[1]->{$val}[3];
			$name =  $adu_arr1[1]->{$val}[0];
			$uid = $adu_arr1[0];
			$shortcode_handling = 1;
			$adunit_position = "";
			$native = $adu_arr1[1]->{$val}[1];
			$displaytype = $adu_arr1[1]->{$val}[2];
			...
			$wpdb->insert(
	       				...
	       				array(
       						...
       						'name'=>$name,
       						'uid' => $uid,
       						'shortcode_handling'=> $shortcode_handling,
